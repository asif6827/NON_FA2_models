...
#sbatch ./data_preprocess/logic/data_preprocess_parsed_v_A_STST.sh
...
echo "Data Pre-Processing Runing"
sbatch ./data_preprocess/logic/data_preprocess_parsed_v_A_MLT_MLT.sh
...
if false; then
    ...
    SYSTEM_NAME_LIST=("Reasoning360_sys_A")
    EVAL_PATH_LIST=("zl_train_zl_test_1_parsed_v_A_${ACRONYM}")
    DATA_PATH_LIST=("ZebraPuzzle_to_guru_parsed_v_A_${ACRONYM}/zl_train_zl_test")
...
if false; then
    ...
    SWITCH_EPOCH_LIST=(80)
    SYSTEM_NAME_LIST=("Reasoning360_sys_A")
    EVAL_PATH_LIST=("med_train_med_test_1_parsed_v_A_${ACRONYM}")
    DATA_PATH_LIST=("ZebraPuzzle_to_guru_parsed_v_A_${ACRONYM}/med_train_med_test")



    SLURM_SCRIPT_A100="./scripts_Qwen3_4B/train/Parsed_v2/${ACRONYM}_1_1shot_A100_zebra_rl_fsdp_parsed_v2.sh"
    for i in "${!TRAIN_TEMP_LIST[@]}"; do
        TRAIN_TEMP=${TRAIN_TEMP_LIST[$i]}
        TEST_TEMP=${TEST_TEMP_LIST[$i]}
...
if false; then
    ...
    SWITCH_EPOCH_LIST=(80)
    SYSTEM_NAME_LIST=("Reasoning360_sys_A")
    EVAL_PATH_LIST=("small_train_small_test_1_parsed_v_A_${ACRONYM}")
    DATA_PATH_LIST=("ZebraPuzzle_to_guru_parsed_v_A_${ACRONYM}/small_train_small_test")
...
if false; then
    ...
    EVAL_PATH_LIST=("small_train_small_test_1_parsed_v5c")
    DATA_PATH_LIST=("ZebraPuzzle_to_guru_parsed_v5c/small_train_small_test")
...
if false; then
    ...
    for i in "${!TRAIN_TEMP_LIST[@]}"; do
        ...
        echo "Submitting job: TRAIN TEMPERATURE=$TEMP, TEST TEMPERATURE=, SCORING-METHOD=$SCORING_METHOD, EPOCH=$EPOCH, TEST-FREQUENCY=$TEST_FREQUENCY, ACC_W=$ACC_W, Z3_W=$Z3_W"
        sbatch $SLURM_SCRIPT_A100 $TRAIN_TEMP $TEST_TEMP $SCORING_METHOD $EPOCH $TEST_FREQUENCY $ACC_W $Z3_W
    done
...
if false; then
    ...
    for i in "${!TRAIN_TEMP_LIST[@]}"; do
        ...
        echo "Submitting job: TEMPERATURE=$TEMP, SCORING-METHOD=$SCORING_METHOD, EPOCH=$EPOCH, TEST-FREQUENCY=$TEST_FREQUENCY, ACC_W=$ACC_W, Z3_W=$Z3_W"
        sbatch $SLURM_SCRIPT_H200 $TRAIN_TEMP $TEST_TEMP $SCORING_METHOD $EPOCH $TEST_FREQUENCY $ACC_W $Z3_W
    done
