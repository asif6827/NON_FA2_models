...
def compute_score(
    ...
    score_method = str(os.environ.get("TEST_SCORE_METHOD", "gt"))
    ...
    try:
        ...
        logger.exception("Failed to Parse Output {e}")
        logger.error(f"Error in Parse Output: {e}")
    ...
    try:
        ...
        logger.exception(f"Error calculating ACC scores: {e}")
        logger.error(f"Error calculating ACC scores: {e}")
        cell_acc_score = 0.0
        puzzle_acc_score = 0.0
        reward = 0.0
        final_result = {}
