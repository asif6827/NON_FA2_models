...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.parquet_files:
            ...
            try:
                if str(parquet_file).endswith('.json') or str(parquet_file).endswith('.jsonl'):
                ...
                import polars as pl
                if str(parquet_file).endswith('.json') or str(parquet_file).endswith('.jsonl'):
                     dataframe = pl.read_ndjson(parquet_file).to_pandas()
                else:
    ...
    def __getitem__(self, item):
        ...
        if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
            print(f"DEBUG-MODE: ROW_DICT = {row_dict}")

        chat = row_dict.pop(self.prompt_key)
        if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
            print(f"DEBUG-MODE: PROMPT-KEY = {self.prompt_key}\n\n")
            print(f"DEBUG-MODE: chat = {chat}\n\n")
            print(f"DEBUG-MODE: chat type = {type(chat)}")
            print(f"DEBUG-MODE: apply_chat_template = {row_dict['apply_chat_template']}")
        ...
        if not isinstance(chat[0], dict):

            prompt_with_chat_template = (
                self.tokenizer.apply_chat_template(chat, add_generation_prompt=False, tokenize=False)
                if row_dict["apply_chat_template"]
                else str(chat)  
            )
        else:
            # 确保 content 是 Python 字符串
            content = chat[0]['content']
            if isinstance(content, np.str_):
                content = str(content)
        ...
        if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
            print(f"DEBUG-MODE: PROMPT WITH CHAT-TEMPLETE = {prompt_with_chat_template}\n\n")
