...
- Reasoning MUST be interleaved:
  The list may contain any number of natural-language reasoning lines and any number of syntactic reasoning steps, in any order.
...
These encode direct facts about house positions.
  ...
  - Ordering:
      ...
      Example:
        "S6: tall < very_short. [C4]"

  - Immediate adjacency:
      x + 1 == y

      Example:
        "S7: baseball + 1 == engineer. [C12]"
...
These operators combine multiple atomic constraints into a single logical statement.
  ...
  - And(e1, e2, ..., en)
      ...
      Example:
        "S8: And(teacher != 2, teacher != 5). [S2+S4]"

  - Or(e1, e2, ..., en)
      At least one sub-constraint must hold.
      This is typically used to represent remaining possible cases.
      Example:
        "S9: Or(teacher == 3, teacher == 4). [S6+S7]"

  - Not(e)
      The negation of a constraint.
      Example:
        "S10: Not(teacher == 1). [S9]"

  - Implies(e1, e2)
      If e1 holds, then e2 must hold.
      This is used to encode conditional deductions during case analysis.
...
- Boolean expressions (Or, And, Implies, Not) MUST be meaningful:
  they should represent genuine case splits, eliminations, or conditional reasoning, not stylistic variation.
