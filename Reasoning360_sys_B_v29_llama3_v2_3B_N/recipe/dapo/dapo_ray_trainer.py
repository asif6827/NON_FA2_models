...
puzzle_key = "val-aux/our_zebra_puzzle_new_reward_test_llama3_v2/PUZZLE_ACCURACY/mean@1"
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                    print(f"DEBUG-MODE: BATCH_DICT = {batch_dict}")
                ...
                with marked_timer("start_profile", timing_raw):
                    if do_profile:
                        if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                            print(f"DEBUG-MODE: DO_PROFILE = {do_profile}")
                        self.actor_rollout_wg.start_profile(role="e2e", profile_step=self.global_steps)
                        if self.use_reference_policy:
                            self.ref_policy_wg.start_profile()
                            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                                print(f"DEBUG-MODE: USE REFERENCE POLICY")
                        if self.use_critic:
                            self.critic_wg.start_profile()
                            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                                print(f"DEBUG-MODE: USING CRITIC")
                        if self.use_rm:
                            self.rm_wg.start_profile()
                            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                                print(f"DEBUG-MODE: USING RM PROFILE")

                new_batch: DataProto = DataProto.from_single_dict(batch_dict)
                if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                    print(f"DEBUG-MODE: NEW BATCH = {new_batch}")
                num_gen_batches += 1
                ...
                if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                    print(f"DEBUG-MODE: GEN_BATCH = {gen_batch}")
