...
def _split_binary_top_level(s: str, op: str):
    ...
    while i <= len(s) - len(op):
        ...
        if ch == '(':
            ...
            left, right = s[:i].strip(), s[i+len(op):].strip()
...
def _parse_expr(expr: str, n_houses: int = 0) -> bool:
    ...
    m = re.match(r"^([A-Za-z_][A-Za-z0-9_]*)\((.*)\)$", e, flags=re.DOTALL)
    ...
    if fn in {'atleast','atmost','exactly'}:
        return len(args) >= 2 and args[0].strip().isdigit() and all(_parse_expr(a, n_houses) for a in args[1:])
    if fn in {'sat','unsat'}:
        return len(args) == 1 and (re.fullmatch(r"Option_[A-Z]", args[0].strip()) is not None or re.fullmatch(r"Not\(\s*Option_[A-Z]\s*\)", args[0].strip()) is not None)
    return False
...
if __name__ == '__main__':
    valid = ['A is not assigned to P1.', 'S1: Not(Assign(A, P1)).', 'Option A is feasible.', 'S2: Sat(Option_A).']
