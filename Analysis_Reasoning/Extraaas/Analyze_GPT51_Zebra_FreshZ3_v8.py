...
def extract_step_referenced_variables(step_text, context):
    """
    Return the payload (category, value) variables referenced by one S_i.

    The symbolic expression is parsed with Python AST. Function names such as
    And/Or/Not/Abs/Implies/Xor are ignored; every remaining ast.Name is
    resolved through the same Z3PuzzleContext used for C_i/S_i parsing.
    """

    expression = strip_symbolic_label(step_text)
    ...
    function_names = {
        "And", "Or", "Not", "Abs", "Implies", "Xor",
    }

    # Reverse lookup from concrete Z3 variable name back to payload value.
    reverse_lookup = {
        str(variable): (category, str(value))
        for (category, value), variable in context.variables.items()
    }
    ...
    for node in ast.walk(parsed.body):
        ...
        if node.id in function_names:
            continue

        variable = context.resolve_name(node.id)
        key = reverse_lookup.get(str(variable))

        if key is None:
            raise ValueError(
                f"Could not map symbolic token '{node.id}' back to a payload value"
            )
...
def table_positions_for_referenced_variables(table, context, referenced_variables):
    """
    Read ONLY the house assignments for variables referenced by S_i.

    Returns:
        (assignments, error)

    where assignments is a list of:
        (z3_variable, house, category, value)

    A variable is considered usable only when the table contains exactly one
    occurrence of that payload value in the mapped category column. Therefore
    malformed predictions with a missing/duplicated relevant value become
    UNVERIFIED rather than being silently repaired.
    """

    if not isinstance(table, dict):
        return None, "table is not a dictionary"

    rows = table.get("rows", []) or []
    house_index = _find_house_column(table)
    category_columns = map_table_columns_to_categories(
        table,
        context.attribute_values,
    )

    if house_index is None:
        return None, "House column could not be identified"

    if category_columns is None:
        return None, "Table columns could not be mapped to payload categories"

    assignments = []

    for category, raw_value in referenced_variables:
        if category not in category_columns:
            return None, f"No table column mapped for category: {category}"

        column_index = category_columns[category]
        target_value = normalize_value(raw_value)
        matching_houses = []
        ...
        for row in rows:
            ...
            if house_index >= len(row) or column_index >= len(row):
                continue

            if normalize_value(row[column_index]) != target_value:
                continue

            try:
                matching_houses.append(int(row[house_index]))
            except (TypeError, ValueError):
                return None, (
                    f"Invalid house value for {category}={raw_value}: "
                    f"{row[house_index]!r}"
                )

        unique_houses = sorted(set(matching_houses))

        if len(unique_houses) != 1:
            return None, (
                f"Expected exactly one house for {category}={raw_value}; "
                f"found {unique_houses if unique_houses else 'none'}"
            )

        z3_variable = context.variables[(category, str(raw_value))]
        assignments.append(
            (z3_variable, unique_houses[0], category, str(raw_value))
        )

    return assignments, None


def check_step_compatibility_with_table(
    base_step_solver,
    context,
    ...
    """
    Formally test whether table values relevant to S_i are compatible with:

        all C_i + S_i

    Only the variables actually mentioned in S_i are pinned from the table.
    This prevents an unrelated wrong prediction cell from making every S_i
    appear prediction-inconsistent.

    Returns:
        (True/False/None, error, assignments_for_log)
    """

    assignments, error = table_positions_for_referenced_variables(
        table,
        context,
        referenced_variables,
    )
    ...
    solver = Solver()
    solver.add(*base_step_solver.assertions())

    assignment_log = []

    for variable, house, category, raw_value in assignments:
        solver.add(variable == house)
    ...
    return solver.check() == sat, None, assignment_log
...
def run_fresh_z3_check(payload, ground_truth, processed_prediction, extra_step=None):
    ...
    BASE (extra_step is None):
        Preserve the original behavior exactly:
        - solve all syntactic clues C_i
        - compare the COMPLETE returned Z3 model with GT / Prediction

    PER-STEP (extra_step is not None):
        Keep C_i + S_i as the formal reasoning context, but do NOT compare the
        complete returned Z3 model with GT / Prediction.

        Instead:
          1. Build and verify SAT(C_i + S_i).
          2. Extract only the variables referenced by S_i.
          3. Test SAT(C_i + S_i + GT assignments for those variables).
          4. Test SAT(C_i + S_i + Prediction assignments for those variables).

        Thus the per-step statuses mean:
          GT-CONSISTENT:
              GT's S_i-relevant values are compatible with C_i + S_i.
          PREDICTION-CONSISTENT:
              Prediction's S_i-relevant values are compatible with C_i + S_i.
    """
    ...
    result = {
        ...
        "comparison_mode": (
            ...
            else "STEP_LOCAL_SAT_COMPATIBILITY"
        ),
        "referenced_variables": [],
        "gt_relevant_assignments": [],
        "prediction_relevant_assignments": [],
        "gt_compatibility_error": None,
        "prediction_compatibility_error": None,
    }
    ...
    if check_result != sat:
        ...
        if isinstance(processed_prediction, dict):
            result["solution_matches_prediction"] = False

        return result

    result["sat"] = True

    # Keep one solution for diagnostics / BASE behavior.
    model = solver.model()
    solution_table = model_to_table(context, model)
    result["solution"] = solution_table

    # ========================================================
    # BASE: preserve original complete-model comparison logic.
    # ========================================================
    if extra_step is None:
    ...
    # ========================================================
    # PER-STEP: step-local solver-backed compatibility.
    # ========================================================
    try:
        referenced_variables = extract_step_referenced_variables(
            extra_step,
            context,
        )
    except Exception as e:
        result["parse_errors"].append(
            {
                "source": "step_variable_extraction",
                "text": extra_step,
                "error": str(e),
            }
        )
    ...
    result["referenced_variables"] = [
        ...
        for category, value in referenced_variables
    ]

    # -------------------------
    # GT compatibility
    # -------------------------
    gt_compatible, gt_error, gt_assignments = (
        check_step_compatibility_with_table(
            solver,
            context,
            referenced_variables,
            ground_truth,
    ...
    result["solution_matches_gt"] = gt_compatible
    result["gt_relevant_assignments"] = gt_assignments
    result["gt_compatibility_error"] = gt_error

    if gt_compatible is True:
        result["gt_status"] = "GT-CONSISTENT"
    elif gt_compatible is False:
        result["gt_status"] = "GT-INCONSISTENT"

    # -------------------------
    # Prediction compatibility
    # -------------------------
    pred_compatible, pred_error, pred_assignments = (
        check_step_compatibility_with_table(
            solver,
            context,
            referenced_variables,
    ...
    result["solution_matches_prediction"] = pred_compatible
    result["prediction_relevant_assignments"] = pred_assignments
    result["prediction_compatibility_error"] = pred_error

    if pred_compatible is True:
        result["prediction_status"] = "PREDICTION-CONSISTENT"
    elif pred_compatible is False:
        result["prediction_status"] = "PREDICTION-INCONSISTENT"
...
def print_original_reasoning_validator_stats(stats):
    ...
    if stats.get("skipped", False):
        print("Validator status                 : SKIPPED")
        print(f"Skip reason                      : {stats.get('skip_reason')}")
        return
...
def analyze_one_example(result_example, puzzle_text, example_number):
    ...
    IMPORTANT BASE-GT GATE
    ------------------------------
    Reasoning analysis is performed ONLY when the fresh clue-only Z3 model
    exactly matches the Ground Truth:

        BASE Z3 Solution == Ground Truth : TRUE

    If this value is FALSE or UNVERIFIED, we do NOT run:
        - the original reasoning/novelty validator
        - the per-step C1...Cn + S_i fresh-Z3 analysis
    ...
    Readable output order:
        ...
        6. Ground truth
        7. Results
        8. Original Z3 reasoning-validator statistics (or SKIPPED)
        9. Fresh Z3 reconstruction
       10. Per-step fresh Z3 checks (or SKIPPED)
    """
    ...
    if payload is None:
        ...
        return {
            ...
            "reasoning_analysis_performed": False,
            "reasoning_analysis_skip_reason": "PAYLOAD_PARSE_ERROR",
        }
    ...
    # ========================================================
    # 3. Complete reasoning (raw model output)
    # ========================================================
    ...
    # ========================================================
    # 7. Compute fresh BASE Z3 silently FIRST.
    #    We need this result before deciding whether any reasoning
    ...
    base_gt_match = (
        base_result.get("solution_matches_gt") is True
    )

    if base_gt_match:
        reasoning_analysis_performed = True
        reasoning_analysis_skip_reason = None

        # Only now call the original reasoning/novelty validator.
        ...
        if base_result.get("solution_matches_gt") is False:
            reasoning_analysis_skip_reason = (
                "BASE_Z3_SOLUTION_DOES_NOT_MATCH_GT"
            )
        else:
            reasoning_analysis_skip_reason = (
                "BASE_Z3_SOLUTION_VS_GT_UNVERIFIED"
            )

        original_validator_stats = {
            "available": False,
            "skipped": True,
            "skip_reason": reasoning_analysis_skip_reason,
            "base_sat_full_GT": None,
            "parse_status": None,
        }
        for key in ORIGINAL_REASONING_STAT_KEYS:
            original_validator_stats[key] = [] if key.startswith("list_") else 0

    # ========================================================
    # 8. Original validator validity / novelty metrics
    #    Printed here to preserve the requested readable order.
    # ========================================================

    print_original_reasoning_validator_stats(original_validator_stats)

    # ========================================================
    # 9. Fresh BASE Z3 reconstruction
    # ========================================================
    ...
    print("\n[REASONING-ANALYSIS GATE]")
    print(
        "Condition                         : "
        "BASE Z3 Solution == Ground Truth"
    )
    print(
        "Gate result                       : "
        f"{bool_text(base_result.get('solution_matches_gt'))}"
    )
    print(
        "Reasoning analysis                : "
        f"{'PERFORMED' if reasoning_analysis_performed else 'SKIPPED'}"
    )
    if reasoning_analysis_skip_reason:
        print(
            "Skip reason                       : "
            f"{reasoning_analysis_skip_reason}"
        )

    # ========================================================
    # 10. Fresh Z3 for each S_i -- ONLY if BASE matches Ground Truth
    # ========================================================
    ...
    if not reasoning_analysis_performed:
        print(
            "SKIPPED: per-step reasoning analysis is only performed when "
            "BASE Z3 Solution == Ground Truth is TRUE."
        )
    else:
            ...
            step_results.append(
                {
                    ...
                    "comparison_mode": step_result.get("comparison_mode"),
                    "referenced_variables": step_result.get("referenced_variables", []),
                    "gt_relevant_assignments": step_result.get(
                        "gt_relevant_assignments", []
                    ),
                    "prediction_relevant_assignments": step_result.get(
                        "prediction_relevant_assignments", []
                    ),
                    "gt_compatibility_error": step_result.get(
                        "gt_compatibility_error"
                    ),
                    "prediction_compatibility_error": step_result.get(
                        "prediction_compatibility_error"
                    ),
                }
            )
    ...
    return {
        ...
        "reasoning_analysis_performed": reasoning_analysis_performed,
        "reasoning_analysis_skip_reason": reasoning_analysis_skip_reason,
        "base_result": {
...
def _print_dataset_z3_stats(stats):
    """
    Print dataset-level reasoning statistics AFTER the BASE-GT gate.

    The A/B/C/D denominator is ONLY the cases satisfying:
    ...
    payload_parse_errors = stats["payload_parse_error_cases"]
    base_gt_true = stats["base_gt_match_true"]
    base_gt_false = stats["base_gt_match_false"]
    base_gt_unverified = stats["base_gt_match_unverified"]
    first_fail_cases = stats["first_failing_reasoning_step_cases"]

    print(f"Original selected cases / errors                    : {selected_cases}")
    print(f"Payload/unparseable cases                           : {payload_parse_errors}")
    print(f"BASE Z3 Solution == Ground Truth : TRUE              : {base_gt_true}")
    print(f"BASE Z3 Solution == Ground Truth : FALSE             : {base_gt_false}")
    print(f"BASE Z3 Solution == Ground Truth : UNVERIFIED        : {base_gt_unverified}")
    print(
        "Cases entering reasoning analysis                     : "
        f"{base_gt_true}"
    )
    ...
    print(
        "CHECK: parse-error + BASE(TRUE/FALSE/UNVERIFIED)       : "
        f"{accounted} / {selected_cases}"
    )
    ...
    if base_gt_true:
        print(
            "Percentage among reasoning-analyzed cases              : "
            f"{100.0 * first_fail_cases / base_gt_true:.2f}%"
        )
    ...
    if first_positions:
        ...
        print("No GT-inconsistent S_i was detected in the analyzed subset.")
    ...
    print("\n" + "=" * 80)
    print("BREAKDOWN OF CASES WITH BASE Z3 SOLUTION == GROUND TRUTH : TRUE")
    print("=" * 80)
    print()
    print(f"Total reasoning-analyzed cases                       : {base_gt_true}")
    print()
    print(
    ...
    print("   └── Every verified S_i is consistent with Ground Truth")
    ...
    print("   └── Every verified S_i is consistent with Ground Truth")
    print("   └── Every verified S_i is also consistent with prediction")
    print()
    print(
    ...
    print(
        "CHECK: A + B + C + D                                    : "
        f"{classified_total} / {base_gt_true}"
    )
...
def analyze_files(
    ...
    dataset_stats = {
        # Overall selected-set accounting.
        "selected_cases": 0,
        "payload_parse_error_cases": 0,

        # HARD GATE for reasoning analysis:
        # only BASE Z3 Solution == Ground Truth : TRUE enters A/B/C/D.
        "base_gt_match_true": 0,
        "base_gt_match_false": 0,
        "base_gt_match_unverified": 0,
        ...
        with (
            ...
            for line_number, line in enumerate(source, start=1):
                ...
                if not analysis.get("payload_parse_ok", False):
                    ...
                    # HARD BASE-GT GATE
                    # -------------------------------------------------
                    # Only TRUE cases are allowed into reasoning analysis
                    # and therefore into the A/B/C/D denominator.
                    # -------------------------------------------------

                    if base_gt_match is True:
                        dataset_stats["base_gt_match_true"] += 1
                    ...
                    elif base_gt_match is False:
                        dataset_stats["base_gt_match_false"] += 1

                    else:
                        dataset_stats["base_gt_match_unverified"] += 1
            ...
            if mode == "errors":
                print(f"Original errors   : {printed}")
