...
def normalize_value(value):
    """
    Canonicalize values for semantic comparison.
...
def make_identifier(value):
    """Convert an attribute value into the identifier form used by C_i/S_i."""
...
def compute_prediction_metrics(ground_truth, prediction, attribute_values):
    """Compute exact puzzle accuracy and category-cell accuracy."""
...
def analyze_files(
    ...
    print readable analysis, and save structured analysis as JSONL.
    """
    ...
    output_txt = output_dir / f"{results_jsonl.stem}_fresh_z3.txt"
    output_jsonl = output_dir / f"{results_jsonl.stem}_fresh_z3_analysis.jsonl"
        ...
        with (
            ...
            for line_number, line in enumerate(source, start=1):
                ...
                if not analysis.get("payload_parse_ok", False):
                    ...
                    if base_result.get("sat") is True:
                        base_sat_true += 1
                    if base_result.get("solution_matches_gt") is True:
                        base_gt_match_true += 1
            ...
            print(f"LLM <answer> JSON parse failures   : {payload_parse_errors}")
            print(f"Fresh BASE SAT == TRUE             : {base_sat_true}")
            print(f"Fresh BASE Z3 solution == GT       : {base_gt_match_true}")
