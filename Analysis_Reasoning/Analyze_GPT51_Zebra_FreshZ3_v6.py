...
def print_original_reasoning_validator_stats(stats):
    ...
    if stats.get("skipped", False):
        ...
        print(f"Reason                           : {stats.get('skip_reason')}")
        return
...
def analyze_one_example(result_example, puzzle_text, example_number):
    ...
    IMPORTANT BASE-PREDICTION GATE
    ------------------------------
    Reasoning analysis is performed ONLY when the fresh clue-only Z3 model
    exactly matches the model prediction:

        BASE Z3 Solution == Prediction : TRUE
    ...
    base_prediction_match = (
        base_result.get("solution_matches_prediction") is True
    )

    if base_prediction_match:
        ...
        if base_result.get("solution_matches_prediction") is False:
            reasoning_analysis_skip_reason = (
                "BASE_Z3_SOLUTION_DOES_NOT_MATCH_PREDICTION"
            )
        else:
            reasoning_analysis_skip_reason = (
                "BASE_Z3_SOLUTION_VS_PREDICTION_UNVERIFIED"
            )
    ...
    print(
        "Condition                         : "
        "BASE Z3 Solution == Prediction"
    )
    print(
        "Gate result                       : "
        f"{bool_text(base_result.get('solution_matches_prediction'))}"
    )
    print(
        "Reasoning analysis                : "
    ...
    # ========================================================
    # 10. Fresh Z3 for each S_i -- ONLY if BASE matches prediction
    # ========================================================
    ...
    if not reasoning_analysis_performed:
        print(
            ...
            "BASE Z3 Solution == Prediction is TRUE."
        )
    else:
...
def _print_dataset_z3_stats(stats):
    """
    Print dataset-level reasoning statistics AFTER the BASE-prediction gate.
    ...
    base_pred_true = stats["base_prediction_match_true"]
    base_pred_false = stats["base_prediction_match_false"]
    base_pred_unverified = stats["base_prediction_match_unverified"]
    ...
    print(f"BASE Z3 Solution == Prediction : TRUE              : {base_pred_true}")
    print(f"BASE Z3 Solution == Prediction : FALSE             : {base_pred_false}")
    print(f"BASE Z3 Solution == Prediction : UNVERIFIED        : {base_pred_unverified}")
    print(
    ...
    if base_pred_true:
        print(
            ...
            f"{100.0 * first_fail_cases / base_pred_true:.2f}%"
        )
    ...
    print("\n" + "=" * 80)
    print("BREAKDOWN OF CASES WITH BASE Z3 SOLUTION == PREDICTION : TRUE")
    print("=" * 80)
    print()
    print(f"Total reasoning-analyzed cases                       : {base_pred_true}")
    print()
    print(
    ...
    print(
        "CHECK: A + B + C + D                                    : "
        f"{classified_total} / {base_pred_true}"
    )
...
def analyze_files(
    ...
    dataset_stats = {
        ...
        # only BASE Z3 Solution == Prediction : TRUE enters A/B/C/D.
        "base_prediction_match_true": 0,
        "base_prediction_match_false": 0,
        "base_prediction_match_unverified": 0,
        ...
        with (
            ...
            for line_number, line in enumerate(source, start=1):
                ...
                if not analysis.get("payload_parse_ok", False):
                    ...
                    base_prediction_match = base_result.get(
                        "solution_matches_prediction"
                    )

                    # -------------------------------------------------
                    # HARD BASE-PREDICTION GATE
                    # -------------------------------------------------
                    # Only TRUE cases are allowed into reasoning analysis
                    # and therefore into the A/B/C/D denominator.
                    # -------------------------------------------------

                    if base_prediction_match is True:
                        dataset_stats["base_prediction_match_true"] += 1
                    ...
                    elif base_prediction_match is False:
                        dataset_stats["base_prediction_match_false"] += 1

                    else:
                        dataset_stats["base_prediction_match_unverified"] += 1
