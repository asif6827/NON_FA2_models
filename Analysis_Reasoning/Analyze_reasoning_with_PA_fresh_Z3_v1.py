...
DEFAULT_LOG_FILE = "gpt51_outputs_test_700_mlxl_nss_temp_0.jsonl"
...
def is_unknown_pa_value(value: Any) -> bool:
    if value is None:
        return True
    return canonicalize_token(value) in {
        canonicalize_token(x) for x in UNKNOWN_PA_VALUES
    }


def ground_truth_by_house(ground_truth: dict):
    if not isinstance(ground_truth, dict):
        return None

    header = ground_truth.get("header", []) or []
    rows = ground_truth.get("rows", []) or []
    house_index = find_house_column(ground_truth)
    ...
    for row in rows:
        ...
        for i, column in enumerate(header):
            if i == house_index:
                continue
            values[canonicalize_token(column)] = row[i]
...
def check_pa_against_gt(pa_table: Any, ground_truth: dict):
    """
    Compare only RESOLVED PA cells against GT.

    '?' / empty / unknown cells are ignored.
    """
    result = {
        "status": "PA-UNVERIFIED",
        "resolved_cells": 0,
        "correct_resolved_cells": 0,
        "incorrect_resolved_cells": 0,
        "errors": [],
        "mismatches": [],
    }

    if not isinstance(pa_table, dict):
        result["errors"].append("PA is not a table dictionary.")
        return result

    pa_header = pa_table.get("header", []) or []
    pa_rows = pa_table.get("rows", []) or []
    gt_header = ground_truth.get("header", []) or []

    if not pa_header or not pa_rows:
        result["errors"].append("PA header/rows are missing or empty.")
        return result

    pa_house_index = find_house_column(pa_table)
    if pa_house_index is None:
        result["errors"].append("PA has no House column.")
        return result

    gt_lookup = ground_truth_by_house(ground_truth)
    if gt_lookup is None:
        result["errors"].append("Ground Truth could not be normalized.")
        return result

    # Map PA columns to GT columns by normalized header name.
    gt_columns = {
        canonicalize_token(column): column
        for column in gt_header
        if canonicalize_token(column) != "house"
    }

    column_map = {}
    for i, column in enumerate(pa_header):
        if i == pa_house_index:
            continue
        normalized = canonicalize_token(column)
        if normalized not in gt_columns:
            result["errors"].append(
                f"PA column {column!r} does not exist in Ground Truth."
            )
            return result
        column_map[i] = normalized

    for row_index, row in enumerate(pa_rows, start=1):
        if not isinstance(row, list) or len(row) != len(pa_header):
            result["errors"].append(
                f"Malformed PA row {row_index}: expected {len(pa_header)} cells."
            )
            return result

        try:
            house = int(row[pa_house_index])
        except (TypeError, ValueError):
            result["errors"].append(
                f"Invalid house value in PA row {row_index}: {row[pa_house_index]!r}"
            )
            return result

        if house not in gt_lookup:
            result["errors"].append(
                f"House {house} does not exist in Ground Truth."
            )
            return result

        for column_index, normalized_column in column_map.items():
            pa_value = row[column_index]

            if is_unknown_pa_value(pa_value):
                continue

            result["resolved_cells"] += 1
            gt_value = gt_lookup[house].get(normalized_column)

            if canonicalize_token(pa_value) == canonicalize_token(gt_value):
                result["correct_resolved_cells"] += 1
            else:
                result["incorrect_resolved_cells"] += 1
    ...
    result["status"] = (
        "PA-CONSISTENT"
        if result["incorrect_resolved_cells"] == 0
        else "PA-INCONSISTENT"
    )
    return result
...
def classify_pa_results(pa_results: list[dict]) -> str:
    if not pa_results:
        return "NO_PA_FOUND"

    statuses = [x.get("status") for x in pa_results]

    if "PA-INCONSISTENT" in statuses:
        return "HAS_PA_NOT_CONSISTENT_WITH_GT"

    if any(status != "PA-CONSISTENT" for status in statuses):
        return "UNRESOLVED_PA"

    return "ALL_PAs_CONSISTENT_WITH_GT"


def combined_category(step_category: str, pa_category: str) -> str:
    if step_category == "HAS_Si_NOT_CONSISTENT_WITH_GT":
        if pa_category == "HAS_PA_NOT_CONSISTENT_WITH_GT":
            return "HAS_BOTH_Si_AND_PA_INCONSISTENCY"
        return "HAS_Si_NOT_CONSISTENT_WITH_GT"
    ...
    if (
        step_category == "ALL_Si_CONSISTENT_WITH_GT"
        and pa_category in ("ALL_PAs_CONSISTENT_WITH_GT", "NO_PA_FOUND")
    ):
        return "ALL_Si_AND_PAs_CONSISTENT_WITH_GT"

    return "UNRESOLVED_Si_OR_PA"
...
def analyze_log(log_path: Path):
    ...
    stats = Counter()
    s_category_counts = Counter()
    pa_category_counts = Counter()
    combined_counts = Counter()

    step_status_counts = Counter()
    first_bad_step_counts = Counter()
    pa_status_counts = Counter()
    ...
    try:
        with log_path.open("r", encoding="utf-8") as src, \
            ...
            print("=" * 120)
            print("STEP-BY-STEP Z3 REASONING ANALYSIS -- LOG WITH PARTIAL ANSWERS")
            print("=" * 120)
            ...
            print("  6. Independently compare every resolved PA cell with Ground Truth.")
            print()
            ...
            for line_number, line in enumerate(src, start=1):
                ...
                if payload is None:
                    ...
                    print(payload_error)
                    print(
                        "\nBASE/STEP/PA ANALYSIS: IGNORED because the full "
                        "reasoning payload could not be recovered."
                    )
                    continue
                ...
                if base_result.get("solution_matches_gt") is not True:
                    ...
                    print(
                        "\nCASE IGNORED FOR REASONING/PA ANALYSIS: "
                ...
                steps, pas = extract_steps_and_pas(reasoning)

                # Some generators may put PAs in a separate top-level field.
                if not pas:
                    top_level_pas = (
                        payload.get("partial_answers")
                        or payload.get("PAs")
                        or payload.get("pas")
                    )
                    if isinstance(top_level_pas, dict):
                        for i, (key, value) in enumerate(top_level_pas.items(), start=1):
                            pas.append({
                                "label": (
                                    str(key).upper()
                                    if re.fullmatch(r"PA\d+", str(key), flags=re.I)
                                    else f"PA{i}"
                                ),
                                "table": value,
                                "reasoning_index": None,
                            })
                    elif isinstance(top_level_pas, list):
                        for i, value in enumerate(top_level_pas, start=1):
                            pas.append({
                                "label": f"PA{i}",
                                "table": value,
                                "reasoning_index": None,
                ...
                # PAs
                # ----------------------------------------------------
                print("\n### PARTIAL ANSWER (PA) ANALYSIS ###")
                pa_results = []

                if not pas:
                    print("No PA entries found.")
                else:
                    for pa in pas:
                        pa_check = check_pa_against_gt(pa["table"], gt)
                        ...
                        print("\n" + "-" * 100)
                        print(f"{pa['label']} | reasoning item {pa.get('reasoning_index')}")
                        print("PA:")
                        print(compact_table_text(pa["table"]))
                        print(f"PA Status                    : {pa_check['status']}")
                        print(f"Resolved cells               : {pa_check['resolved_cells']}")
                        print(
                            "Correct resolved cells       : "
                            f"{pa_check['correct_resolved_cells']}"
                        )
                        print(
                            "Incorrect resolved cells     : "
                            f"{pa_check['incorrect_resolved_cells']}"
                        )

                        if pa_check["mismatches"]:
                            print("Mismatches:")
                            for mismatch in pa_check["mismatches"]:
                                print(
                                    f"  House {mismatch['house']} | "
                                    f"{mismatch['column']} | "
                                    f"PA={mismatch['pa_value']!r} | "
                                    f"GT={mismatch['gt_value']!r}"
                                )

                        if pa_check["errors"]:
                            print("PA check errors:")
                            for error in pa_check["errors"]:
                                print(f"  - {error}")

                # Aggregate S_i and PA-level statistics.
                for step_result in step_results:
                    step_status_counts[step_result.get("status", "UNVERIFIED")] += 1

                for pa_result in pa_results:
                    pa_status_counts[pa_result.get("status", "PA-UNVERIFIED")] += 1
                    stats["pa_resolved_cells"] += int(
                        pa_result.get("resolved_cells", 0) or 0
                    )
                    stats["pa_correct_resolved_cells"] += int(
                        pa_result.get("correct_resolved_cells", 0) or 0
                    )
                    stats["pa_incorrect_resolved_cells"] += int(
                        pa_result.get("incorrect_resolved_cells", 0) or 0
                    )

                s_category = classify_step_results(step_results)
                pa_category = classify_pa_results(pa_results)
                overall = combined_category(s_category, pa_category)

                s_category_counts[s_category] += 1
                pa_category_counts[pa_category] += 1
                combined_counts[overall] += 1

                print("\n### CASE CATEGORIES ###")
                print(f"S_i category   : {s_category}")
                print(f"PA category    : {pa_category}")
                ...
                first_bad_pa = next(
                    (
                        x for x in pa_results
                        if x.get("status") == "PA-INCONSISTENT"
                    ),
                    None,
                )
                if first_bad_pa:
                    print(
                        "First PA not consistent with GT: "
                        f"{first_bad_pa['label']}"
                    )
            ...
            print("\nS_i categories after BASE-GT gate:")
            for category in (
                ...
                print(f"  {category:<46}: {s_category_counts[category]}")

            print("\nPA categories after BASE-GT gate:")
            for category in (
                "ALL_PAs_CONSISTENT_WITH_GT",
                "HAS_PA_NOT_CONSISTENT_WITH_GT",
                "UNRESOLVED_PA",
                "NO_PA_FOUND",
            ):
                print(f"  {category:<46}: {pa_category_counts[category]}")

            print("\nCombined categories:")
            for category in (
                "ALL_Si_AND_PAs_CONSISTENT_WITH_GT",
                "HAS_Si_NOT_CONSISTENT_WITH_GT",
                "HAS_PA_NOT_CONSISTENT_WITH_GT",
                "HAS_BOTH_Si_AND_PA_INCONSISTENCY",
                "UNRESOLVED_Si_OR_PA",
            ):
                print(f"  {category:<46}: {combined_counts[category]}")
            ...
            all_s_consistent = s_category_counts["ALL_Si_CONSISTENT_WITH_GT"]
            has_s_inconsistent = s_category_counts["HAS_Si_NOT_CONSISTENT_WITH_GT"]
            unresolved_s = (
                s_category_counts["UNRESOLVED_Si"]
                + s_category_counts["UNRESOLVED_NO_Si"]
            )

            all_pa_consistent = pa_category_counts["ALL_PAs_CONSISTENT_WITH_GT"]
            has_pa_inconsistent = pa_category_counts["HAS_PA_NOT_CONSISTENT_WITH_GT"]
            unresolved_pa = pa_category_counts["UNRESOLVED_PA"]
            no_pa = pa_category_counts["NO_PA_FOUND"]
            ...
            print("\nCase-level PA consistency:")
            if eligible_cases:
                print(
                    "  All PAs consistent with GT                        : "
                    f"{all_pa_consistent} "
                    f"({100.0 * all_pa_consistent / eligible_cases:.2f}%)"
                )
                print(
                    "  >=1 PA inconsistent with GT                       : "
                    f"{has_pa_inconsistent} "
                    f"({100.0 * has_pa_inconsistent / eligible_cases:.2f}%)"
                )
                print(
                    "  Unresolved PA                                     : "
                    f"{unresolved_pa} "
                    f"({100.0 * unresolved_pa / eligible_cases:.2f}%)"
                )
                print(
                    "  No PA found                                       : "
                    f"{no_pa} "
                    f"({100.0 * no_pa / eligible_cases:.2f}%)"
                )

            total_pas = sum(pa_status_counts.values())
            print("\nPA-level results:")
            print(f"  Total PAs checked                                 : {total_pas}")
            print(f"  PA-CONSISTENT                                     : {pa_status_counts['PA-CONSISTENT']}")
            print(f"  PA-INCONSISTENT                                   : {pa_status_counts['PA-INCONSISTENT']}")
            print(f"  PA-UNVERIFIED                                     : {pa_status_counts['PA-UNVERIFIED']}")

            resolved_cells = stats["pa_resolved_cells"]
            correct_cells = stats["pa_correct_resolved_cells"]
            incorrect_cells = stats["pa_incorrect_resolved_cells"]

            print("\nResolved PA-cell results:")
            print(f"  Total resolved PA cells                           : {resolved_cells}")
            if resolved_cells:
                print(
                    "  Correct resolved PA cells                         : "
                    f"{correct_cells} "
                    f"({100.0 * correct_cells / resolved_cells:.2f}%)"
                )
                print(
                    "  Incorrect resolved PA cells                       : "
                    f"{incorrect_cells} "
                    f"({100.0 * incorrect_cells / resolved_cells:.2f}%)"
                )

            print("\nCombined case categories:")
            for category in (
                "ALL_Si_AND_PAs_CONSISTENT_WITH_GT",
                "HAS_Si_NOT_CONSISTENT_WITH_GT",
                "HAS_PA_NOT_CONSISTENT_WITH_GT",
                ...
                value = combined_counts[category]
                percentage = (
                    100.0 * value / eligible_cases
                    if eligible_cases else 0.0
                )
                print(f"  {category:<46}: {value} ({percentage:.2f}%)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Fresh-Z3 step + PA analysis for a log WITH Partial Answers."
    )
    parser.add_argument(
        "--log",
