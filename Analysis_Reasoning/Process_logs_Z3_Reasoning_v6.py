...
def extract_step_symbol_names(text):
    """
    Return the symbolic attribute-value names referenced by one S_i.

    Example:
        S4: And(Alice == 4, basketball == Alice)
        -> ["Alice", "basketball"]

    Logical helper names such as And/Or/Not are excluded.
    """
    ...
    for node in ast.walk(parsed):
        ...
        if name in logical_function_names:
            continue
...
def table_to_value_positions(table):
    """
    Convert a concrete Zebra table (GT or Prediction) into:

        normalized attribute value -> house number

    Only unambiguous values are retained. Surface forms such as
    ``hip hop`` and ``hip_hop`` map to the same normalized token.

    The House column itself is ignored.
    """
    ...
    for column_index, column_name in enumerate(header):
        if normalize_value(column_name) == "house":
            house_index = column_index
            break
    ...
    for token, houses in temporary.items():
        ...
        # Keep only values whose table position is unambiguous.
        if len(unique_houses) == 1:
            positions[token] = next(iter(unique_houses))
...
def check_step_compatibility_with_table(
    context,
    clue_plus_step_solver,
    step_text,
    table,
):
    """
    Formal step-local consistency check.

    Starting context is ALWAYS:

        base Zebra constraints + all C_i + the current S_i

    We then pin ONLY the variables explicitly referenced by S_i to their
    positions in the supplied concrete table (GT or Prediction), and ask Z3
    whether that augmented constraint set is SAT.

    Returns:
        (True,  None)  -> table values appearing in S_i are compatible
        (False, None)  -> table values appearing in S_i contradict C + S_i
        (None,  error) -> the check cannot be performed reliably
    """

    positions = table_to_value_positions(table)

    if positions is None:
        return None, "Reference table is missing or cannot be parsed"

    try:
        referenced_names = extract_step_symbol_names(step_text)
    except Exception as e:
        return None, str(e)

    if not referenced_names:
        return None, "No symbolic attribute-value names found in S_i"

    local_solver = Solver()
    local_solver.add(*clue_plus_step_solver.assertions())
    ...
    for name in referenced_names:
        ...
        if token not in positions:
            return None, (
                f"Referenced S_i token '{name}' was not found unambiguously "
                "in the reference table"
            )

        try:
            variable = context.resolve_name(name)
        except Exception as e:
            return None, str(e)

        local_solver.add(variable == positions[token])

    return local_solver.check() == sat, None
...
def run_fresh_z3_check(
    ...
    BASE check (extra_step is None):
        - Build syntactic clues C_i only.
        - Solve once and compare the COMPLETE returned Z3 model with GT and
          processed_prediction. This preserves the original BASE behavior.

    Per-S_i check (extra_step is not None):
        - Build the formal context: C_1 ... C_n + ONE S_i.
        - Confirm that C + S_i itself is SAT.
        - Extract only the symbolic values referenced by S_i.
        - GT consistency asks whether C + S_i remains SAT when those S_i
          variables are fixed to their GT positions.
        - Prediction consistency asks the same question using the positions
          asserted by the final prediction.

    Thus an unrelated wrong cell elsewhere in the prediction cannot make an
    otherwise preserved S_i look prediction-inconsistent.
    """
    ...
    # Never solve a weakened constraint set if a requested C_i/S_i failed.
    if parse_errors:
        return result
    ...
    # Keep one representative model available for diagnostics.
    model = solver.model()
    ...
    # ========================================================
    # BASE: preserve original complete-model comparison.
    # ========================================================
    ...
    # ========================================================
    # PER-STEP: C + S_i + relevant GT / Prediction values.
    # ========================================================
    try:
        result["referenced_step_symbols"] = extract_step_symbol_names(
            extra_step
        )
    except Exception as e:
        # Parsing should normally already have succeeded above, but retain a
        # safe UNVERIFIED path rather than guessing.
        result["gt_compatibility_error"] = str(e)
        result["prediction_compatibility_error"] = str(e)
        return result

    gt_match, gt_error = check_step_compatibility_with_table(
        context=context,
        clue_plus_step_solver=solver,
        step_text=extra_step,
        table=ground_truth,
    )

    prediction_match, prediction_error = check_step_compatibility_with_table(
        context=context,
        clue_plus_step_solver=solver,
        step_text=extra_step,
        table=processed_prediction,
    )

    # Keep the old keys so ALL downstream A/B/C/D logic remains unchanged.
    # For S_i these now mean step-local solver compatibility, NOT whole-table
    # equality with the representative model returned above.
    result["solution_matches_gt"] = gt_match
    result["solution_matches_prediction"] = prediction_match
    result["gt_compatibility_error"] = gt_error
    result["prediction_compatibility_error"] = prediction_error
...
def run_per_step_z3_analysis(example, payload):
    ...
    for step in symbolic_steps:
        ...
        if result.get("gt_compatibility_error"):
            print(
                "GT compatibility note           : "
                f"{result.get('gt_compatibility_error')}"
            )

        if result.get("prediction_compatibility_error"):
            print(
                "Prediction compatibility note   : "
                f"{result.get('prediction_compatibility_error')}"
            )
        ...
        step_results.append(
            {
                ...
                "referenced_step_symbols": result.get("referenced_step_symbols", []),
                "gt_compatibility_error": result.get("gt_compatibility_error"),
                "prediction_compatibility_error": result.get(
