...
def _print_dataset_z3_stats(stats):
    ...
    aligned_pred_inconsistent = stats[
        "all_reasoning_gt_consistent_prediction_inconsistent_cases"
    ]
    ...
    print("\nGT-CONSISTENT REASONING / PREDICTION-INCONSISTENT CASES")
    print(
        "Cases where ALL verified S_i are GT-CONSISTENT "
        "and at least one S_i is PREDICTION-INCONSISTENT : "
        f"{aligned_pred_inconsistent}"
    )

    if base_true:
        print(
            "Percentage of base_sat_full_GT=True cases            : "
            f"{100.0 * aligned_pred_inconsistent / base_true:.2f}%"
        )

    print(
        "Cases with at least one individual S_i that is both "
        "GT-CONSISTENT and PREDICTION-INCONSISTENT               : "
        f"{stats['has_gt_consistent_prediction_inconsistent_step_cases']}"
    )

    print("\nOTHER DIAGNOSTIC COUNTS")
    print(
        "Cases with all S_i GT-CONSISTENT                         : "
        f"{stats['all_reasoning_gt_consistent_cases']}"
    )
    print(
...
def read_jsonl(
    ...
    dataset_stats = {
        ...
        "all_reasoning_gt_consistent_cases": 0,
        "all_reasoning_gt_consistent_prediction_inconsistent_cases": 0,
        "has_gt_consistent_prediction_inconsistent_step_cases": 0,
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        for line_number, line in enumerate(f, start=1):
            ...
            # Only cases gated by logged base_sat_full_GT=True
            # can return fresh_analysis.
            # =================================================

            if logged_base_sat_full_gt is True and fresh_analysis is not None:
                ...
                if not step_results:
                    ...
                    # -----------------------------------------
                    # 1. First S_i for which clues + S_i is
                    #    GT-INCONSISTENT.
                    # -----------------------------------------
                    ...
                    # -----------------------------------------
                    # 2. Strict case-level reasoning-to-answer
                    #    inconsistency: every S_i is aligned
                    #    with GT, but at least one S_i is not
                    #    aligned with processed_prediction.
                    # -----------------------------------------
                    ...
                    if all_gt_consistent:
                        dataset_stats[
                            "all_reasoning_gt_consistent_cases"
                        ] += 1

                        if "PREDICTION-INCONSISTENT" in pred_statuses:
                            dataset_stats[
                                "all_reasoning_gt_consistent_prediction_inconsistent_cases"
                            ] += 1

                    # Also expose the looser witness-level statistic:
                    # at least one S_i simultaneously agrees with GT
                    # and disagrees with prediction.
                    if any(
                        step.get("gt_status") == "GT-CONSISTENT"
                        and step.get("prediction_status")
                        == "PREDICTION-INCONSISTENT"
                        for step in step_results
                    ):
                        dataset_stats[
                            "has_gt_consistent_prediction_inconsistent_step_cases"
                        ] += 1

                    if "UNVERIFIED" in gt_statuses:
                        dataset_stats["gt_unverified_step_cases"] += 1
