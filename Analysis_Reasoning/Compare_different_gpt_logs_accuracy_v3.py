...
def write_log_as_txt(result, output_dir):
    ...
    with output_path.open("w", encoding="utf-8") as out:
        for i, case in enumerate(result["cases"]):
            ...
            out.write(f"Status: {case['status']}\n")
            out.write(
            ...
            out.write("\nGROUND TRUTH\n")
            out.write("-" * 120 + "\n")
            out.write(pretty_json(case["ground_truth"]) + "\n")

            out.write("\nEXTRACTED FINAL SOLUTION\n")
            out.write("-" * 120 + "\n")
            out.write(pretty_json(case["prediction"]) + "\n")
...
def write_error_difference_file(
    ...
    with output_path.open("w", encoding="utf-8") as out:
        ...
        for rank, case_id in enumerate(cases, start=1):
            ...
            out.write("\nGROUND TRUTH\n")
            out.write("-" * 120 + "\n")
            out.write(pretty_json(original["ground_truth"]) + "\n")

            out.write("\nORIGINAL EXTRACTED SOLUTION\n")
            out.write("-" * 120 + "\n")
            out.write(pretty_json(original["prediction"]) + "\n")

            out.write("\nNSS EXTRACTED SOLUTION\n")
            out.write("-" * 120 + "\n")
            out.write(pretty_json(nss["prediction"]) + "\n")
...
def compare_logs(original_file, nss_file, output_dir="./Outputs"):
    original_result = analyze_log(original_file)
    nss_result = analyze_log(nss_file)
