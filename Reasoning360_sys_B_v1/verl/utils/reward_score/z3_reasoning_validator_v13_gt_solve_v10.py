...
def count_distinct_reasoning_steps_v13_relaxed(
    ...
    *,
    ...
    after_first_contradiction_steps: List[dict] = []
    first_contradiction_seen: bool = False
    ...
    for raw in reasoning_lines:
        ...
        # If we've already hit a contradiction in the reasoning chain, steps can still be VALID,
        # but they must not be counted as novel (and are tagged AFTER_FIRST_CONTRADICTION).
        after_contradiction = bool(first_contradiction_seen)
        ...
        # If this step is after the first contradiction, we still score validity vs clues,
        # but we exclude it from all novelty accounting and do NOT add it to Γ_steps.
        if after_contradiction:
            ...
            entry = {"k": k, "raw": raw, "expr": expr, "status": "AFTER_FIRST_CONTRADICTION", "error": None}
            after_first_contradiction_steps.append(entry)
            skipped_steps_inc_clues.append(entry)
            skipped_steps_exc_clues.append(entry)
        ...
        # If the current step contradicts clues, this is the first contradiction point (if not already seen).
        # The contradiction step itself is NOT counted as novel, and all subsequent steps are tagged AFTER_FIRST_CONTRADICTION.
        if validity_status == "CONTRADICTION":
            first_contradiction_seen = True
            entry = {"k": k, "raw": raw, "expr": expr, "status": "CONTRADICTION", "error": None}
            skipped_steps_inc_clues.append(entry)
            skipped_steps_exc_clues.append(entry)
            seen_step_sexprs.add(phi_sexpr)
        ...
        if steps_status == "CONTRADICTION":
            entry = {"k": k, "raw": raw, "expr": expr, "status": "CONTRADICTION", "error": None}
            # Mark the first contradiction in the chain; all subsequent steps are tagged and excluded.
            first_contradiction_seen = True
            skipped_steps_exc_clues.append(entry)
            skipped_steps_inc_clues.append(entry)
            seen_step_sexprs.add(phi_sexpr)
    ...
    return {
        ...
        "n_steps_after_first_contradiction": len(after_first_contradiction_steps),
...
if __name__ == "__main__":
    ...
    reasoning_lines = [
        ...
        "S5: Eric == 1.",  # ENTAILED (from S3 + S4)
        "S1: 2 == Arnold.",  # ENTAILED (from C1 + C2)
        "S5: Eric == 1.",  # ENTAILED (from S3 + S4)
        "S2: Not(Eric == 1).",           # ENTAILED (from C4 + 3-house domain)
        "S3: Or(Eric ==1, Eric == 2, Eric == 3).", # ENTAILED (from S2 + domain)
        "S4: Not(Eric == 2).",           # ENTAILED (since Arnold==2 and names are Distinct)
        "S5: Eric == 1.",                # ENTAILED (from S3 + S4)
        "S6: Eric == 3.",  # ENTAILED (from S3 + S4)
        "S7: Eric == 1.",  # ENTAILED (from S3 + S4)
        # ---- negative / filtering tests ----
        "S8: Arnold == 2.",              # DUPLICATE_STEP
        "S9: red == 2.",                 # RESTATES_CLUE (exact match to C2)
        "S10: Or(eric ==1, Eric ==, Eric == 3).",   # PARSE_ERROR
    ]
