...
def extract_reasoning_and_solution(solution_str: str):
    ...
    if answer_content:
        ...
        if parsed is not None:
            return parsed.get("syntactic_clues", None), parsed.get("reasoning", None), parsed.get("self_verification", None), parsed.get("solution", None), parsed.get("attribute_values", None), parsed.get("n_houses",
                                                                                                                                                                          None), "success_answer_tag"
        return None, None, None, None, None, None, "answer_tag_json_error"
    ...
    return None, None, None, None, None, None, "parsing_failed"
...
def compute_score(
    ...
    try:
        parsed_clues, parsed_reasoning, verification, predicted_arrangement, attribute_values, n_houses, parse_status = extract_reasoning_and_solution(solution_str=solution_str)
    ...
    try:
        ...
        if attribute_values is not None and n_houses is not None:
            normalizer =  max(2 * int(n_houses) * int(len(attribute_values.keys())),1)
