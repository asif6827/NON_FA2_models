...
def _trim_dataproto_to_multiple_of_chunks(dp, chunks: int):
    size = len(dp)

    # if empty, just return as is (avoid crash)
    if size == 0:
        print("[WARN] Empty batch encountered. Skipping trim.")
        return dp

    # if smaller than chunks → pad instead of failing
    if size < chunks:
        print(f"[WARN] Batch too small ({size}). Padding to {chunks}.")
        pad_needed = chunks - size
        pad_part = dp[:pad_needed]
        return dp.union(pad_part)

    remainder = size % chunks

    if remainder != 0:
        valid_size = size - remainder
        print(f"[WARN] Trimming batch from {size} to {valid_size} for equal chunking.")
        dp = dp[:valid_size]
