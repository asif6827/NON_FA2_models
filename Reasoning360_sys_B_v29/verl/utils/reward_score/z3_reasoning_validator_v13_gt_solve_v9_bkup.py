...
def _extract_sid_and_expr(sid: str, value: str) -> Tuple[int, str]:
    m = _SID_RE.fullmatch(sid.strip())
    if not m:
        return -1, str(value).strip()

    return int(m.group(1)), str(value).strip().rstrip(".").strip()
...
def count_distinct_reasoning_steps_v13_relaxed(
    ...
    *,
    ...
    for sid, raw in reasoning_lines.items():
        k, expr = _extract_sid_and_expr(sid, raw)
        if k == -1:
            continue
