# -*- coding: utf-8 -*-
"""
check_interleved_format_v9_binary_nlspa.py
...
9. Every PA must resolve at least one non-House cell; an all-'?' PA is
   not a meaningful checkpoint.
10. After the last PA, at least one complete NL/S pair must be emitted before
...
def check_interleaved_reasoning_detailed(
    ...
    *,
    ...
    # Every PA checkpoint must contain at least one resolved non-House cell.
    pa_content_errors: List[Dict[str, Any]] = []
    if isinstance(reasoning, dict):
        for key, value in reasoning.items():
            if not _PA_RE.fullmatch(str(key)):
                continue
            ...
            if resolved == 0:
                pa_content_errors.append({
                    "key": str(key),
                    "code": "PA_EMPTY",
                    "message": (
                        f"{key} must resolve at least one non-House cell; "
                        "an all-'?' PA is not a meaningful checkpoint."
                    ),
                })

    schema_errors.extend(pa_content_errors)

    schema["schema_ok_without_interleave"] = bool(
        schema.get("ok", False) and not pa_content_errors
    )
...
if __name__ == "__main__":
    ...
    expected = {
        ...
        "pa_at_end": False,
        "empty_pa": True,  # placement is valid; PA content makes overall ok=False
