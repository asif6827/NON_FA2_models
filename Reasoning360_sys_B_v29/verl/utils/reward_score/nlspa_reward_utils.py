...
if __name__ == "__main__":
    # Regression example for the NEW NL_i / S_i / PA_i schema.
    n_houses = 3
    attribute_values = {
    ...
    schema = validate_reasoning_schema(
        reasoning,
        n_houses=n_houses,
        expected_header=solution["header"],
        attribute_values=attribute_values,
    )
    pa = score_partial_answers(
        reasoning,
        n_houses=n_houses,
        expected_header=solution["header"],
        attribute_values=attribute_values,
    ...
    assert schema["n_nl"] == 9
    assert schema["n_s"] == 9
    assert schema["n_pa"] == 2
    assert pa["pa_cell_precision"] == 1.0
    assert pa["pa_prefix_support_score"] == 1.0
    assert pa["pa_monotonicity_score"] == 1.0

    print("NL/S/PA utility regression test: PASS")
    print("legacy S-lines:", reasoning_to_legacy_s_lines(reasoning))
    print("PA PRM:", pa["pa_prm_score"])
