...
def main_task(config):
    ...
    print('Batch Size: {}'.format(config_batch_size))
    print('Num Batches: {}'.format(num_batch))
    ...
    for batch_idx in range(num_batch):
        ...
        if os.environ.get("USE_Thinking", "0") == "1":
            print('Using Qwen Thinking Model')
            ...
            print('Using Qwen Non-Thinking Model')
            inputs = tokenizer.apply_chat_template(
                batch_chat_lst,
                add_generation_prompt=True,
                padding=True,
        ...
        print("shape of processed inputs = {}".format(input_ids.shape))
        attention_mask = inputs["attention_mask"]
        #print("attention-mask = {}".format(attention_mask))
        position_ids = compute_position_id_with_mask(attention_mask)
        #print("position-ids = {}".format(position_ids))
        batch_dict = {"input_ids": input_ids, "attention_mask": attention_mask, "position_ids": position_ids}
        #batch_dict_wo_pos = {"input_ids": input_ids, "attention_mask": attention_mask}

        data = DataProto.from_dict(batch_dict)
        #data = DataProto.from_dict(batch_dict_wo_pos)
