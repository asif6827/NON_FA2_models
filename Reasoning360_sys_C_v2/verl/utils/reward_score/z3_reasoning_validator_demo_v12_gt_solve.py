...
def validate_reasoning_steps(
    *,
    ...
    try:
        ...
        # --- If FULL constraints are SAT, extract the Z3-solved solution table (for GT validation)
        z3_solution: Optional[Dict[str, Any]] = None
        if full_sat is True:
            try:
                z3_solution = _z3_model_to_solution_table(model.solver.model(), model)
            except Exception:
                z3_solution = None
        ...
        gt_valid, gt_details = validate_solution_against_ground_truth(
            ...
            predicted_solution=z3_solution,
...
def main():
    # Local import to avoid circular dependency when this module is imported by our_puzzles_dataset
...
if __name__ == "__main__":
    main()
# ----------------------- Z3 model -> solution table -------------------------

def _z3_model_to_solution_table(model: Any, z3puz: Z3PuzzleModel) -> Dict[str, Any]:
    """
    Build a 'solution' table from the satisfying Z3 model.

    Output format matches your dataset convention:
      {
        "header": ["House", <Attr1>, <Attr2>, ...],
        "rows": [[ "1", v_for_attr1, v_for_attr2, ...], ...]
      }

    If the model is partial/unknown for a cell, uses "UNKNOWN".
    """
    attrs = list(z3puz.attribute_values.keys())
    header = ["House"] + attrs

    # Evaluate all (attr,value) -> house
    av_house: Dict[Tuple[str, str], int] = {}
    for a, vals in z3puz.attribute_values.items():
        for v in vals:
            try:
                hv = model.eval(z3puz.var[a][v], model_completion=True)
                av_house[(a, v)] = int(str(hv))
            except Exception:
                # leave missing
                continue

    rows: List[List[str]] = []
    for h in range(1, int(z3puz.n) + 1):
        row = [str(h)]
        for a in attrs:
            chosen = "UNKNOWN"
            for v in z3puz.attribute_values[a]:
                if av_house.get((a, v)) == h:
                    chosen = str(v)
                    break
            row.append(chosen)
        rows.append(row)
