...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
            print(f"DEBUG-MODE: Start Validating Configration...!")
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        print(f"LENGTH OF TRAIN DATA SET = {len(self.train_dataset)}\n")
        print(f"LENGTH OF VALIDATION DATA SET = {len(self.val_dataset)}\n")
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path):
        ...
        if os.environ.get("STEP1_STATUS", "0") == "1":
            filename = os.path.join(dump_path, f"{self.global_steps}_valid_step1.jsonl")
        else:
            filename = os.path.join(dump_path, f"{self.global_steps}_valid_step2.jsonl")
    ...
    def _dump_generations_tr(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path):
        ...
        os.makedirs(dump_path, exist_ok=True)
        if os.environ.get("STEP1_STATUS", "0") == "1":
            filename = os.path.join(dump_path, f"{self.global_steps}_train_step1.jsonl")
        else:
            filename = os.path.join(dump_path, f"{self.global_steps}_train_step2.jsonl")
    ...
    def init_workers(self):
        ...
        if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
            print(f"DEBUG-MODE: INITILIZING-WORKERS")

        # create actor and rollout
        if self.hybrid_engine:
            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                print(f"DEBUG-MODE: USING HYBRID-MODE")
        ...
        if self.use_critic:
            ...
            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                print(f"DEBUG-MODE: WORKING CRITIC..!")
        else:
            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                print(f"DEBUG-MODE: NOT WORKING CRITIC..!")

        # create reference policy if needed
        if self.use_reference_policy:
            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                print(f"DEBUG-MODE: USING REFERENCE-POLICY")
        ...
        if self.use_critic:
            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                print(f"DEBUG-MODE: WORKING WITH CRITIC..!")
            self.critic_wg = all_wg["critic"]
            self.critic_wg.init_model()

        if self.use_reference_policy and not self.ref_in_actor:
            if os.environ.get("DEBUG_CODE", "0").lower() in ("1", "true", "yes"):
                print(f"DEBUG-MODE: WORKING WITH REF POLICY..!")
    ...
    def _load_checkpoint(self):
        if self.config.trainer.resume_mode == "disable":
            print(" Resume from checkpoint dis-abled. Training from scratch")
            return 0
        ...
        if self.config.trainer.default_hdfs_dir is not None:
            ...
            print('Global step folder is', global_step_folder)
