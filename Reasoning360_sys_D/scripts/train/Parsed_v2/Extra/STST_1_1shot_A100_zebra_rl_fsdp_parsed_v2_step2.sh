...
#SBATCH -J tr_A100_STST_1_1_5B_1S_step2 #job name
...
WANDB_PROJECT="Zebra_logic_Qwen2_5_1_5B_STST_1_1shot_step2" # Your wandb project name
...
export TEST_SCORE_METHOD='gt+z3'
...
export REWARD_LOG_PATH=/export/home/asifali/Reasoning360/evaluation_results/small_train_small_test_1_parsed_v2_step2/$MODEL_NAME
export PUZZLE_FEEDBACK_PATH=/export/home/asifali/Reasoning360/feedback_results/small_train_small_test_1_parsed_v2_step2/$MODEL_NAME

# =================== Data Mixture ===================
SHARED_DATA_PATH=/export/home/asifali/HF_cache/ZebraPuzzle_to_guru_parsed_v2/small_train_small_test
VALID_GENERATION_PATH=/export/home/asifali/Reasoning360/evaluation_results/small_train_small_test_1_parsed_v2_step2/$MODEL_NAME
...
if [[ -n "$RESUME_CKPT_DIR_NAME" ]]; then
    ...
    WANDB_EXPERIMENT_NAME="single-node-${TIMESTAMP}-${BASE_MODEL##*/}-step2"
fi
...
# =================== Step-2 Specific Configs ===================
# 步骤2数据生成和使用相关配置
STEP2_DATA_DIR=/export/home/asifali/Reasoning360/step2_data/small_train_small_test_1_parsed_v2_step2/$MODEL_NAME
STEP2_PROCESSOR_SCRIPT=/export/home/asifali/Reasoning360/verl/utils/reward_score/our_step2_data_WWQ.py
STEP2_DATA_PROCESSOR=/export/home/asifali/Reasoning360/verl/utils/reward_score/step2_data_processor.py
STEP2_DATASET=/export/home/asifali/Reasoning360/verl/utils/reward_score/step2_dataset.py

# 创建步骤2数据目录
mkdir -p ${STEP2_DATA_DIR}
mkdir -p ${PUZZLE_FEEDBACK_PATH}

# =================== Start RL training with Step-2 support ===================
# Ensure your python environment (e.g., conda) is activated before running this script.
echo "Starting training with Step-2 support..."

# 主训练循环，支持多轮epoch
for ((epoch=1; epoch<=$EPOCH; epoch++)); do
    echo "\n\n====================================="
    echo "Starting epoch $epoch/$EPOCH"
    echo "=====================================\n\n"
    
    # 步骤1: 运行初始训练，生成feedback.jsonl
    echo "Running Step-1 training..."
    python -m recipe.dapo.main_dapo \
        ...
        trainer.experiment_name=${WANDB_EXPERIMENT_NAME}-epoch${epoch}-step1 \
        ...
        reward_model.reward_score.meta="{\"epoch\": ${epoch}, \"enable_step_feedback\": true}"
    
    # 立即更新BASE_MODEL为Step 1的checkpoint
    echo "\n\nFinding Step 1 checkpoint..."
    STEP1_CKPT=$(find ${VALID_GENERATION_PATH} -name "checkpoint-*" -type d -printf "%T@ %p\n" | sort -nr | head -1 | cut -d' ' -f2-)
    
    if [ -n "$STEP1_CKPT" ]; then
        echo "✓ Updating BASE_MODEL to Step 1 checkpoint: $STEP1_CKPT"
        BASE_MODEL=$STEP1_CKPT
    else
        echo "⚠ No checkpoint found after Step 1 training!"
        echo "  Continuing with original BASE_MODEL: $BASE_MODEL"
    fi
    
    # 步骤2: 生成步骤2数据
    echo "\n\nGenerating Step-2 data..."
    
    # 找到最新的feedback.jsonl文件
    LATEST_FEEDBACK=$(find ${PUZZLE_FEEDBACK_PATH} -name "*.jsonl" -type f -printf "%T@ %p\n" | sort -nr | head -1 | cut -d' ' -f2-)
    
    if [ -z "$LATEST_FEEDBACK" ]; then
        echo "ERROR: No feedback.jsonl file found in ${PUZZLE_FEEDBACK_PATH}"
        exit 1
    fi
    
    echo "Using latest feedback file: $LATEST_FEEDBACK"
    
    # 生成步骤2数据
    STEP2_OUTPUT=${STEP2_DATA_DIR}/step2_data_epoch${epoch}.jsonl
    python ${STEP2_PROCESSOR_SCRIPT} \
        --feedback_jsonl ${LATEST_FEEDBACK} \
        --output_jsonl ${STEP2_OUTPUT} \
        --mode parsed_only
    
    if [ $? -ne 0 ]; then
        echo "ERROR: Step-2 data generation failed"
        exit 1
    fi
    
    echo "Step-2 data generated: ${STEP2_OUTPUT}"
    
    # 步骤3: 使用步骤2数据进行DAPO更新
    echo "\n\nRunning DAPO update with Step-2 data..."
    
    # 显示当前使用的模型
    echo "✓ Using model for Step 2: $BASE_MODEL"
    
    # 更新train_files为步骤2生成的数据
    STEP2_TRAIN_FILES="['${STEP2_OUTPUT}']"
    
    # 运行DAPO更新，使用Step 1的checkpoint作为基础
    python -m recipe.dapo.main_dapo \
        --config-path=config \
        --config-name="dapo_fsdp_config.yaml" \
        algorithm.adv_estimator=${adv_estimator} \
        ...
        algorithm.filter_groups.metric=${filter_groups_metric} \
        algorithm.filter_groups.max_num_gen_batches=${max_num_gen_batches} \
        data.train_files="$STEP2_TRAIN_FILES" \
        data.val_files="$test_files" \
        data.prompt_key=prompt \
        data.truncation='right' \
        data.max_prompt_length=${max_prompt_length} \
        ...
        trainer.logger=['console','wandb'] \
        trainer.project_name=${WANDB_PROJECT} \
        trainer.experiment_name=${WANDB_EXPERIMENT_NAME}-epoch${epoch}-step2 \
        trainer.val_before_train=True \
        trainer.n_gpus_per_node=${NUM_GPUS} \
        trainer.nnodes=1 \
        trainer.save_freq=100 \
    ...
    # 更新BASE_MODEL为当前epoch的检查点，以便下一轮使用
    LATEST_CKPT=$(find ${VALID_GENERATION_PATH} -name "checkpoint-*" -type d -printf "%T@ %p\n" | sort -nr | head -1 | cut -d' ' -f2-)
    
    if [ -n "$LATEST_CKPT" ]; then
        echo "\n\nUpdating BASE_MODEL to latest checkpoint: $LATEST_CKPT"
        BASE_MODEL=$LATEST_CKPT
    fi
done
