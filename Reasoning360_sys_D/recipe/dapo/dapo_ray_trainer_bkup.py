...
迭代训练框架：
- 一个完整EPOCH = Step 1 + Step 2
- Step 2可以连续迭代：Step 2 → Step 2 → Step 2 → ...
- 每次Step 2使用累积的反馈历史
"""
...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    Iterative Training Framework Design:
    - A complete EPOCH = Step 1 + Step 2_iterations
    - Step 1: Raw data → LLM generation → Z3 validation → Feedback to disk (JSONL/Parquet)
    - Step 2: Build feedback prompt in memory → LLM regeneration → Z3 validation
    - Multiple Step 2 iterations utilize the accumulated feedback history
    """

    def fit(self):
        """
        The training loop of PPO with iterative Step-2 training.
        """
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                if enable_step2 and step2_iterations > 0 and "step_verification_data" in step1_reward_extra_infos:
                    ...
                    feedback_path = os.path.join(file_path, f"jobid_{job_id}_feedback.jsonl")
                    ...
                    train_feedback_path = save_dataset(dataset=feedback_data, output_dir=file_path,
                            ...
                            with marked_timer("reward", timing_raw, "yellow"):
                                ...
                                if STEP2_SCORINING:
                                    ...
                                    if self.config.algorithm.use_kl_in_reward:
                                        new_batch, kl_metrics = apply_kl_penalty(
                                            new_batch, kl_ctrl=self.kl_ctrl_in_reward, kl_penalty=self.config.algorithm.kl_penalty)
                                        feedback_metrics.update(kl_metrics)
