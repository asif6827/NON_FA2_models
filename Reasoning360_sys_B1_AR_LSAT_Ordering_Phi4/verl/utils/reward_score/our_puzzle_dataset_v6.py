...
Updated for AR-LSAT ordering runs where the model often emits:
...
# Kept for backward compatibility, but this script now uses an AR-LSAT ordering
...
RESULT_KEYS = [
    ...
    "raw_s_step_parse_rate", "ordering_format_reward", "selected_option_test_ok",
...
def _normalize_ordering_payload(payload: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    normalized["problem_type"] = "ordering"
    return normalized
...
def _infer_n_positions(payload: Optional[Dict[str, Any]]) -> Optional[int]:
    ...
    if isinstance(domains, dict):
        preferred_keys = ("positions", "position", "slots", "ranks", "orders", "values", "domains")
    ...
    wm_entities = wm.get("entities", []) if isinstance(wm, dict) else []
    if isinstance(wm_entities, list) and wm_entities:
        return len(wm_entities)
...
FORMAL_FIELD_RE = re.compile(
    r"("
    r"Assign\s*\(|Distinct\s*\(|Not\s*\(|And\s*\(|Or\s*\(|Implies\s*\(|Xor\s*\(|"
    r"Exactly\s*\(|AtLeast\s*\(|AtMost\s*\(|Sat\s*\(|Unsat\s*\(|"
    r"\b[A-Za-z_][A-Za-z0-9_]*\s*(?:[+\-]\s*\d+\s*)?(?:==|!=|<=|>=|<|>)\s*(?:[A-Za-z_][A-Za-z0-9_]*|\d+)(?:\s*[+\-]\s*\d+)?"
    r")"
)
...
ORDERING_ATOM = r"(?:[A-Za-z_][A-Za-z0-9_]*|\d+)(?:\s*[+\-]\s*\d+)?"
ORDERING_CMP = rf"{ORDERING_ATOM}\s*(?:==|!=|<=|>=|<|>)\s*{ORDERING_ATOM}"

FORMAL_STEP_RE = re.compile(
    rf"""^S\d+:\s*(
        (Assign|Distinct|Not|And|Or|Implies|Xor|Exactly|AtLeast|AtMost|Sat|Unsat)\s*\(.*\)
        |
        {ORDERING_CMP}
    )\.$""",
    re.VERBOSE,
...
def _ordering_reasoning_stats(reasoning: Any) -> Dict[str, float]:
    out = {
...
def compute_score(solution_str, ground_truth, extra_info: Any = None, score_method: str = "gt", timeout: float = 3.0, acc_weight: float = 0.8, clue_weight: float = 1.0, z3_weight: float = 0.2, meta: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
    try:
        ...
        payload = _normalize_ordering_payload(payload)
        ...
        reasoning_stats = _ordering_reasoning_stats(reasoning)
        out.update(reasoning_stats)
        out["format_reward"] = out["format_status_ok"] = out["ordering_format_reward"]
        ...
        if isinstance(payload, dict) and schema_reward > 0.0:
            ...
            z3_payload["problem_type"] = "ordering"
        ...
        if n_positions is not None and n_entities is not None:
        ...
        logger.exception("ordering compute_score failed; returning complete penalty reward dict")
...
def _make_ordering_answer(selected: str = "A", bad_format: bool = False, close: bool = True) -> str:
    """Small self-test payload for AR-LSAT ordering reward sanity checks."""
    reasoning = [
...
def _make_ordering_must_be_true_answer() -> str:
    payload = {
        "world_model": {
            "entities": ["A", "B", "C"],
        ...
        "reasoning": [
            "A is fixed in the first position by the first ordering rule.",
            "S1: A == 1.",
            "Option A is forced in every valid ordering.",
...
def _make_missing_schema_answer() -> str:
    payload = {
        "solution": {"selected_option": "A"},
        "reasoning": ["Option A is feasible.", "S1: Sat(Option_A)."],
    }
    return _wrap(payload)
...
if __name__ == "__main__":
    malformed_domains_payload = {
        ...
        "options": {"A": "A == 1", "B": "A == 2"},
        "reasoning": [
            "A is fixed in the first position by the first rule.",
            "S1: A == 1.",
    ...
    tests = [
        ("correct_could_be_true", _make_ordering_answer("A"), "A"),
        ("open_answer_json_no_close", _make_ordering_answer("A", close=False), "A"),
        ("wrong_selected_option", _make_ordering_answer("B"), "A"),
        ("bad_format_correct_answer", _make_ordering_answer("A", bad_format=True), "A"),
        ("must_be_true", _make_ordering_must_be_true_answer(), "A"),
        ("malformed_domains_list_no_crash", _wrap(malformed_domains_payload), "A"),
        ("malformed_json", "<answer>{this is not valid json}</answer>", "A"),
        ("missing_answer_tag_direct_json", json.dumps(json.loads(find_last_answer_block(_make_ordering_answer("A")))), "A"),
