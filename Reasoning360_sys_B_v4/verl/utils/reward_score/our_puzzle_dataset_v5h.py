...
from verl.utils.reward_score.z3_reasoning_validator_v13_verifier_GT_clues import validate_reasoning_and_solution
...
def compute_score(
    ...
    # --------------- Validate Reasoning -----------
    reasoning_valid_out = {"reward": 0.0}
    if parsed_reasoning and predicted_arrangement and parsed_clues:
        try:
            reasoning_valid_out = validate_reasoning_and_solution(parsed_clues, parsed_reasoning, predicted_arrangement, ground_truth)
        except Exception as reasoning_error:
            logger.error(f"Error in self-verification: {reasoning_error}")
    else:
        reasoning_valid_out = {"reward" : 0.0}
    ...
    try:
        ...
        #    normalizer =  3 * max(int(n_houses) * int(len(attribute_values.keys())),1)
        ...
        #reward = reward  + reasoning_valid_out["reward"]

        validator_score = (reasoning_valid_out["reward"] + 2.0) / 3.0
        validator_score = max(0.0, min(1.0, validator_score))
        if attribute_values is not None and n_houses is not None:
            normalizer = max(int(n_houses) * int(len(attribute_values.keys())), 1)
            steps_ratio = final_result["pass_rate_strict_gt_validated"]
            steps_ratio = max(0.0, min(1.0, steps_ratio))

            if puzzle_acc_score == 0:
                reward = -0.5 + 0.1 * steps_ratio
            else:
                reward = (
                        0.6
                        + 0.2 * steps_ratio
    ...
    if os.environ.get("VALID_STATUS", "0") == "1":
        ...
        try:
            ...
            example_["reasoning_validator_out"] = reasoning_valid_out
            if norm_pred != None:
