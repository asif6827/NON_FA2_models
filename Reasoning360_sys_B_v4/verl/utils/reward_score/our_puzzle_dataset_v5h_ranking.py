...
# ----------------------------- Ranking-style reward helpers -----------------------------

def _safe_div(num: float, den: float) -> float:
    return float(num) / float(den) if float(den) != 0.0 else 0.0


def _solution_cell_accuracy(pred_sol: Any, ref_sol: Any) -> float:
    """Compute cell-level accuracy between two solution tables (dict with header/rows).
    Uses House as row key when present; otherwise compares row-wise.
    Returns 0.0 on any format issue.
    """
    try:
        if not isinstance(pred_sol, dict) or not isinstance(ref_sol, dict):
            return 0.0
        ph, pr = pred_sol.get("header"), pred_sol.get("rows")
        rh, rr = ref_sol.get("header"), ref_sol.get("rows")
        if not (isinstance(ph, list) and isinstance(pr, list) and isinstance(rh, list) and isinstance(rr, list)):
            return 0.0

        pred_sol_n = normalize_header({"header": ph, "rows": pr})
        ref_sol_n = normalize_header({"header": rh, "rows": rr})
        ph, pr = pred_sol_n.get("header"), pred_sol_n.get("rows")
        rh, rr = ref_sol_n.get("header"), ref_sol_n.get("rows")
        if not (ph and pr and rh and rr):
            return 0.0

        # build house-indexed maps when possible
        def to_house_map(h, rows):
            if "House" in h:
                hi = h.index("House")
                m = {}
                for row in rows:
                    if isinstance(row, list) and len(row) == len(h):
                        m[str(row[hi])] = row
                return m, hi
            return None, None

        pmap, phi = to_house_map(ph, pr)
        rmap, rhi = to_house_map(rh, rr)

        # align columns by header name intersection (excluding House)
        cols = [c for c in ph if c in rh and c != "House"]
        if not cols:
            return 0.0
        ...
        if pmap is not None and rmap is not None:
            houses = sorted(set(pmap.keys()) & set(rmap.keys()), key=lambda x: int(x) if x.isdigit() else x)
            if not houses:
                return 0.0
            for house in houses:
                prow = pmap[house]
                rrow = rmap[house]
                pc = row_cells(ph, prow)
                rc = row_cells(rh, rrow)
                for c in cols:
                    total += 1
                    if str(pc.get(c)) == str(rc.get(c)):
                        correct += 1
        else:
            # fallback row-wise
            n = min(len(pr), len(rr))
            for i in range(n):
                if not (isinstance(pr[i], list) and isinstance(rr[i], list)):
                    continue
                pc = row_cells(ph, pr[i])
                rc = row_cells(rh, rr[i])
                for c in cols:
                    total += 1
                    if str(pc.get(c)) == str(rc.get(c)):
                        correct += 1
...
def compute_reasoning_quality_score(
    z3_out: Dict[str, Any],
    predicted_solution_norm: Optional[Dict[str, Any]],
) -> Tuple[float, Dict[str, float]]:
    """Reasoning-first score for groupwise ranking (8 samples/puzzle).

    This score intentionally rewards:
    - parsable syntactic steps
    - strict entailment and chain entailment
    - distinct steps (anti-repetition)
    - low contradiction in chain
    - solution linkage (model solution matches the Z3-implied solution)

    Returns:
      (Q, metrics_dict)
    """
    n_total = float(z3_out.get("n_steps_total") or 0.0)
    n_parsed = float(z3_out.get("n_steps_parsed_ok") or 0.0)
    n_strict = float(z3_out.get("n_steps_entailed_strict") or 0.0)
    n_contra = float(z3_out.get("n_steps_contradiction_chain") or 0.0)

    distinct_blob = z3_out.get("log_compute_distinct_steps") or {}
    n_distinct = float(distinct_blob.get("n_steps_distinct") or 0.0)

    parse_rate = _safe_div(n_parsed, n_total) if n_total > 0 else 0.0
    strict_rate = _safe_div(n_strict, n_total) if n_total > 0 else 0.0
    contra_rate = _safe_div(n_contra, n_total) if n_total > 0 else 0.0
    distinct_rate = _safe_div(n_distinct, n_total) if n_total > 0 else 0.0

    # Linkage: compare model's solution to solver's derived solution (already normalized in validator).
    z3_solution = z3_out.get("z3_solution")
    sol_cell_acc = 0.0
    if predicted_solution_norm is not None and isinstance(z3_solution, dict):
        sol_cell_acc = _solution_cell_accuracy(predicted_solution_norm, z3_solution)
    solution_link = 1.0 if sol_cell_acc >= 0.999 else 0.0

    # Hard gates to kill junk quickly
    # (These are especially useful early in training.)
    gate_penalty = 0.0
    if n_parsed <= 0:
        gate_penalty -= 2.0
    if n_total > 0 and strict_rate < 0.05 and parse_rate > 0.0:
        # parsed but mostly not entailed => likely nonsense
        gate_penalty -= 0.5
    if solution_link <= 0.0:
        gate_penalty -= 2.0
    if contra_rate > 0.0:
        gate_penalty -= 1.0 * min(1.0, contra_rate)

    # Weighted score (tune weights later; ranking will reduce sensitivity)
    Q = (
        1.5 * strict_rate
        + 1.0 * distinct_rate
        + 0.8 * parse_rate
    ...
    metrics = {
        ...
        "solution_link": float(solution_link),
        "sol_cell_acc_vs_z3": float(sol_cell_acc),
...
def compute_score(
    ...
    final_result["BASE_n_steps_distinct"] = 0.0
    ...
    try:
        ...
        #verification = parsed_reasoning["Self_Verification"]
    ...
    if n_houses and attribute_values and parsed_clues and parsed_reasoning:
        ...
        final_result["BASE_n_steps_total"] = z3_out["log_compute_distinct_steps"]["n_steps_total"]
        final_result["BASE_n_steps_parsed_ok"] = z3_out["log_compute_distinct_steps"]["n_steps_parsed_ok"]
        final_result["BASE_n_steps_distinct"] = z3_out["log_compute_distinct_steps"]["n_steps_distinct"]
    ...
    try:
        ...
        # Ranking-style reward (best for 8 samples/group). This reward is meant to be
        # normalized/ranked across the 8 samples by the RL algorithm (e.g., GRPO).
        #
        # You can fall back to the legacy ACC-heavy reward by setting:
        #   export USE_LEGACY_ACC_REWARD=1
        use_legacy = os.environ.get("USE_LEGACY_ACC_REWARD", "0").lower() in ("1","true","yes")

        if attribute_values is not None and n_houses is not None:
            if use_legacy:
                normalizer = 5 * max(int(n_houses) * int(len(attribute_values.keys())), 1)
                reward = 0.6 * puzzle_acc_score + 0.4 * (final_result["n_steps_entailed_strict"] / normalizer)
                #final_result["reward_mode"] = "legacy_acc"
            else:
                reward, rq = compute_reasoning_quality_score(
                    z3_out=z3_out,
                    predicted_solution_norm=norm_pred if 'norm_pred' in locals() else None,
                )
                #final_result["reward_mode"] = "ranking_reasoning"
                # Log reasoning-quality sub-metrics for debugging/analysis
                #for k, v in rq.items():
                #    final_result[f"RQ_{k}"] = v
        else:
            reward = 0.0
            #final_result["reward_mode"] = "missing_domain"
    ...
    if os.environ.get("VALID_STATUS", "0") == "1":
        ...
        try:
            ...
            example_["self_verification"] = verification
