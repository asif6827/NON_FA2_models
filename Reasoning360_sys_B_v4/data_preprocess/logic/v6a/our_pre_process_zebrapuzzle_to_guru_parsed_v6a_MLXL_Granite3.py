...
def serialize_granite_messages(messages, add_generation_prompt=True):
    """
    Manual serializer for IBM Granite Instruct chat format.
    """
    ...
    for message in messages:
        ...
        if role not in {"system", "user", "assistant"}:
            raise ValueError(f"Unsupported Granite role: {role}")

        parts.append(
            f"<|start_of_role|>{role}<|end_of_role|>"
            f"{content}"
            "<|end_of_text|>\n"
        )
...
def make_map_fn_1_shot(split, data_source):
    def process_fn_1_shot(example, idx):
        ...
        target_attribute_values = attribute_values_from_solution(final_grid)

        user_prompt = (
            SOLUTION_PROMPT_1_SHOT_SYS.strip()
            + "\n\n"
        ...
        granite_prompt = serialize_granite_messages(
            messages,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_source_train', default='our_zebra_puzzle_new_reward_Granite3', help='Name of data source')
    parser.add_argument('--data_source_test', default='our_zebra_puzzle_new_reward_test_Granite3', help='Name of data source')
